...
<body
  ...
  <div class="w-full max-w-md bg-cardbg rounded-3xl shadow-sm border border-chartbg/50 p-8 sm:p-10 relative z-10">
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-4">
      @csrf
      <div>
        <label for="nama" class="block text-xs font-bold text-textmuted mb-2 uppercase tracking-wider">Nama
          Lengkap</label>
        <input type="text" id="nama" name="nama_lengkap" value="{{ old('nama_lengkap') }}"
          placeholder="ex: Heru Kristanto"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label for="password_confirmation"
            class="block text-xs font-bold text-textmuted mb-2 uppercase tracking-wider">Konfirmasi</label>
    ...
    <div class="mt-6 text-center border-t border-chartbg/50 pt-5">
      <p class="text-sm font-medium text-textmuted">Sudah punya akun? <a href="{{ route('login') }}"
          class="text-accent font-bold hover:underline">Masuk</a></p>
    </div>
  </div>
</body>
